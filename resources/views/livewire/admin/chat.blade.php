    ...
    <div class="container-fluid">
        ...
        <div class="card elevation-0">
            <div class="card-header">
                <h3 class="card-title"><i class="fas fa-comments"></i> Chats</h3>
                {{-- <div class="card-tools">
                    <a href="{{ route('admin.chat.create')}}" class="btn btn-sm btn-info"><i class="fas fa-plus-circle"></i> Create chat</a>
                </div> --}}
            </div>
            {{-- <div class="card-body">
                <div class="row">
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            @forelse ($chats as $key => $chat)
                                <tr>
                                    <td>{{$key+1}}</td>
                                    {{-- <td><span class="badge badge-info">{{ $chat->date}}</span></td> --}}
                                    <td>{{ $chat->title}}</td>
                                    <td>{{ $chat->description}}</td>
                                    <td>
                                        {{-- <a href="{{ route('admin.chat.invoice', $chat) }}" class="btn btn-outline-info btn-xs">
                                            <i class="fas fa-receipt"></i> Invoice
                                        </a>

                                        <a href="{{ route('admin.chat.edit', $chat) }}" class="btn btn-primary btn-xs">
                                            <i class="fas fa-edit"></i> Edit
                                        </a> --}}
                                        
                                        <button type="button" data-target="#modal-del{{ $chat->id }}"
                                            data-toggle="modal" class="btn btn-xs btn-danger">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                        
                                        <div id="modal-del{{ $chat->id }}" class="modal fade" role="dialog">
                                            <div class="modal-dialog modal-md">
                                                <div class="modal-content">
                                                    ...
                                                    <div class="modal-body">
                                                        <p>Do you really wish to delete this chat? This will delete permanently !</p>
                                                    </div>
                                                    <div class="modal-footer">
                                                        <form action="{{route('admin.chat.delete' , $chat)}}"
                                                            method="post">
                                                            @csrf
                                                            @method('delete')
                                        ...
                                        {{-- <a href="{{ route('admin.chat.detail', $chat) }}" class="btn btn-info btn-xs">
